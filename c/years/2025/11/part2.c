...
int dfs_paths(conn_t *conns, size_t lc, char *cur, char *target, char **path, int depth,
              bool seen_dac, bool seen_fft) {
  path[depth] = cur;
  if (strcmp(cur, "dac") == 0) seen_dac = true;
  if (strcmp(cur, "fft") == 0) seen_fft = true;
  ...
  for (int i = 0; i < c->count; i++) {
    ...
    total += dfs_paths(conns, lc, nxt, target, path, depth + 1, seen_dac, seen_fft);
  }
  return total;
}
...
int main() {
  ...
  int count = dfs_paths(conns, lc, "svr", "out", pathbuf, 0, false, false);
